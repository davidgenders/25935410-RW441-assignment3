...
class ClassificationTuner:
    ...
    def evaluate_config_cv_active(self, dataset: str, strategy: str, method: str, lr: float, wd: float, 
        ...
        for trial in range(N_TRIALS):
            ...
            for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
                # Note: For active learning, we need to be careful about the data split
                # This is a simplified version - in practice, you might want to use the full dataset
                # and simulate the active learning process within each fold
                
                # For now, we'll use the experiments module which handles the full dataset
                if fold == 0:  # Only run once per trial to avoid redundancy
                    train_config = TrainConfig(learning_rate=lr, weight_decay=wd, batch_size=bs, 
                                             max_epochs=200, patience=20, device='cpu')
                    active_config = ActiveConfig(initial_labeled=init, query_batch=query, 
                                               max_labels=budget, device='cpu')
                    
                    if strategy == 'uncertainty':
                        res = run_active_classification(dataset_name=dataset, strategy='uncertainty', 
                                                       uncertainty_method=method, hidden_units=hidden, 
                                                       train_config=train_config, active_config=active_config)
                    elif strategy == 'sensitivity':
                        res = run_active_classification(dataset_name=dataset, strategy='sensitivity', 
                                                       hidden_units=hidden, train_config=train_config, 
                                                       active_config=active_config)
                    
                    trial_metrics.append(res)
        
        # For active learning, we run fewer evaluations due to computational cost
